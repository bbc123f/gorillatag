...
public class FusionStatsMeterBar : FusionGraphBase
{
	...
	public void LateUpdate()
	{
		...
		double num = Math.Max(this._currentBarValue - (double)(Time.deltaTime / this.DecayTime) * this._max, 0.0);
		this.SetBar(num);
	}
	...
	public void GenerateMeter()
	{
		...
		this.Bar.fillMethod = Image.FillMethod.Horizontal;
		this.Bar.fillAmount = 0f;
		RectTransform rectTransform3 = base.transform.CreateRectTransform("Label", true).ExpandAnchor(null).SetAnchors(0f, 0.5f, 0f, 1f)
			.SetOffsets(6f, -6f, 6f, -6f);
		this.LabelTitle = rectTransform3.AddText(description.LongName, TextAnchor.MiddleLeft, this._fusionStats.FontColor);
		this.LabelTitle.alignByGeometry = false;
		RectTransform rectTransform4 = base.transform.CreateRectTransform("Value", true).ExpandAnchor(null).SetAnchors(0.5f, 1f, 0f, 1f)
			.SetOffsets(6f, -6f, 6f, -6f);
		this.ValueLabel = rectTransform4.AddText("0.0", TextAnchor.MiddleRight, this._fusionStats.FontColor);
