...
internal class RoomSystem : MonoBehaviour, IInRoomCallbacks, IMatchmakingCallbacks
{
	internal static void DeserializeLaunchProjectile(object[] projectileData, PhotonMessageInfo info)
	{
		...
		int projectileCount = (int)projectileData[3];
		bool overridecolour = (bool)projectileData[4];
		VRRig rig = rigContainer.Rig;
		if (rig.isOfflineVRRig || rig.CheckDistance(position, 4f))
		{
			...
			RoomSystem.launchProjectile.colour = new Color32(r, g, b, a);
	...
	internal static void DeserializeImpactEffect(object[] impactData, PhotonMessageInfo info)
	{
		...
		if (!position.IsValid() || !float.IsFinite(num) || !float.IsFinite(num2) || !float.IsFinite(num3) || !float.IsFinite(num4))
		{
			GorillaNot.instance.SendReport("invalid impact state", info.Sender.UserId, info.Sender.NickName);
			return;
		}
		int projectileCount = (int)impactData[5];
	...
	private static void StaticLoad()
	{
		...
		RoomSystem.netEventCallbacks[4] = new Action<object[], PhotonMessageInfo>(RoomSystem.SearchForNearby);
		RoomSystem.netEventCallbacks[7] = new Action<object[], PhotonMessageInfo>(RoomSystem.SearchForParty);
	...
	private static void OnEvent(EventData data)
	{
		...
		try
		{
			...
			int timestamp = (int)array[0];
			byte key = (byte)array[1];
			object[] arg = null;
			if (array.Length > 2)
			{
				object obj = array[2];
			...
			PhotonMessageInfo arg2 = new PhotonMessageInfo(player, timestamp, null);
	...
	internal static void SearchForNearby(object[] shuffleData, PhotonMessageInfo info)
	{
		...
		if (PlayFabAuthenticator.instance.GetSafety() || !NetworkSystem.Instance.SessionIsPrivate)
		{
			return;
		}
		PhotonNetworkController.Instance.AttemptToFollowIntoPub(info.Sender.UserId, info.Sender.ActorNumber, newKeyStr, shufflerStr, JoinType.FollowingNearby);
	}

	internal static void SearchForParty(object[] shuffleData, PhotonMessageInfo info)
	{
		...
		{
			"SearchForParty: Received shuffler ",
			text,
			" key ",
			text2,
			" sender ",
	...
	internal static void SendPartyFollowCommand(string shuffler, string keyStr)
	{
		...
		{
			if (vrrig.IsLocalPartyMember && vrrig.creator != PhotonNetwork.LocalPlayer)
			{
				raiseEventOptions.TargetActors[0] = vrrig.creator.ActorNumber;
	...
	private static void DeserializeStatusEffect(object[] data, PhotonMessageInfo info)
	{
		...
		RoomSystem.StatusEffects obj = (RoomSystem.StatusEffects)data[0];
	...
	private static void DeserializePlayerEffect(object[] data, PhotonMessageInfo info)
	{
		...
		RoomSystem.PlayerEffect effect = (RoomSystem.PlayerEffect)data[0];
		Player target = (Player)data[1];
