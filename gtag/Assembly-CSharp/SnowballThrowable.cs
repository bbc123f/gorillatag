...
public class SnowballThrowable : HoldableObject
{
	public event SnowballThrowable.SnowballHitEvent OnSnowballHitPlayer
	{
		...
		add
		{
			...
			do
			{
				...
				SnowballThrowable.SnowballHitEvent value2 = (SnowballThrowable.SnowballHitEvent)Delegate.Combine(snowballHitEvent2, value);
				snowballHitEvent = Interlocked.CompareExchange<SnowballThrowable.SnowballHitEvent>(ref this.OnSnowballHitPlayer, value2, snowballHitEvent2);
			}
		...
		remove
		{
			...
			{
				snowballHitEvent2 = snowballHitEvent;
				SnowballThrowable.SnowballHitEvent value2 = (SnowballThrowable.SnowballHitEvent)Delegate.Remove(snowballHitEvent2, value);
				snowballHitEvent = Interlocked.CompareExchange<SnowballThrowable.SnowballHitEvent>(ref this.OnSnowballHitPlayer, value2, snowballHitEvent2);
			}
			while (snowballHitEvent != snowballHitEvent2);
		}
	...
	private void LaunchSnowball()
	{
		...
		Vector3 velocity = a + vector + b;
		Color32 throwableProjectileColor = this.targetRig.GetThrowableProjectileColor(this.isLeftHanded);
		if (PhotonNetwork.InRoom)
		{
			int projectileCount = ProjectileTracker.IncrementLocalPlayerProjectileCount();
			component.Launch(base.transform.position, velocity, PhotonNetwork.LocalPlayer, false, false, projectileCount, x, this.randomizeColor, throwableProjectileColor);
		...
		{
			component.Launch(position, velocity, PhotonNetwork.LocalPlayer, false, false, 0, x, this.randomizeColor, throwableProjectileColor);
		}
