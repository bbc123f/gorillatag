...
public class NetworkDebugStart : Fusion.Behaviour
{
	...
	public Task AddClient(GameMode serverMode, SceneRef sceneRef)
	{
		...
		Task result = this.InitializeNetworkRunner(networkRunner, gameMode, NetAddress.Any(0), sceneRef, null);
	...
	private sealed class <StartWithClients>d__54 : IEnumerator<object>, IEnumerator, IDisposable
	{
		...
		bool IEnumerator.MoveNext()
		{
			...
			case 0:
			{
				...
				serverTask = networkDebugStart.InitializeNetworkRunner(networkDebugStart._server, serverMode, NetAddress.Any(networkDebugStart.ServerPort), sceneRef, new Action<NetworkRunner>(NetworkDebugStart.<>c.<>9.<StartWithClients>b__54_0));
				break;
			}
			case 1:
				this.<>1__state = -1;
