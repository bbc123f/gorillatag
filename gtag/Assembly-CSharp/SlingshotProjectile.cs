...
public class SlingshotProjectile : MonoBehaviour
{
	public event SlingshotProjectile.ProjectileHitEvent OnHitPlayer
	{
		...
		add
		{
			...
			do
			{
				...
				SlingshotProjectile.ProjectileHitEvent value2 = (SlingshotProjectile.ProjectileHitEvent)Delegate.Combine(projectileHitEvent2, value);
				projectileHitEvent = Interlocked.CompareExchange<SlingshotProjectile.ProjectileHitEvent>(ref this.OnHitPlayer, value2, projectileHitEvent2);
			}
		...
		remove
		{
			...
			{
				projectileHitEvent2 = projectileHitEvent;
				SlingshotProjectile.ProjectileHitEvent value2 = (SlingshotProjectile.ProjectileHitEvent)Delegate.Remove(projectileHitEvent2, value);
				projectileHitEvent = Interlocked.CompareExchange<SlingshotProjectile.ProjectileHitEvent>(ref this.OnHitPlayer, value2, projectileHitEvent2);
			}
			while (projectileHitEvent != projectileHitEvent2);
		}
	...
	public void Launch(Vector3 position, Vector3 velocity, Player player, bool blueTeam, bool orangeTeam, int projectileCount, float scale, bool shouldOverrideColor = false, Color32 overrideColor = default(Color32))
	{
