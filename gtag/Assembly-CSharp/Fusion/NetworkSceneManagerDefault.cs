	...
	public class NetworkSceneManagerDefault : NetworkSceneManagerBase
	{
		...
		protected virtual IEnumerator SwitchSceneSinglePeer(SceneRef prevScene, SceneRef newScene, NetworkSceneManagerBase.FinishedLoadingDelegate finished)
		{
			...
			else
			{
				...
				yield return this.LoadSceneAsync(newScene, parameters, delegate(Scene scene)
				{
					loadedScene = scene;
				});
				if (!loadedScene.IsValid())
			...
			List<NetworkObject> sceneObjects = base.FindNetworkObjects(loadedScene, true, false);
			finished(sceneObjects);
			yield break;
		}
		...
		private sealed class <SwitchSceneMultiplePeer>d__4 : IEnumerator<object>, IEnumerator, IDisposable
		{
			...
			bool IEnumerator.MoveNext()
			{
				...
				this.<>2__current = networkSceneManagerDefault.LoadSceneAsync(newScene, loadSceneParameters, new Action<Scene>(CS$<>8__locals1.<SwitchSceneMultiplePeer>b__0));
				this.<>1__state = 3;
				return true;
				IL_261:
				finished(sceneObjects);
			...
			bool IEnumerator.MoveNext()
			{
				...
				case 0:
				{
					...
					if (!(prevScene == default(SceneRef)) || !networkSceneManagerDefault.IsScenePathOrNameEqual(activeScene, newScene))
					{
						LoadSceneParameters parameters = new LoadSceneParameters(LoadSceneMode.Single);
						CS$<>8__locals1.loadedScene = default(Scene);
						this.<>2__current = networkSceneManagerDefault.LoadSceneAsync(newScene, parameters, new Action<Scene>(CS$<>8__locals1.<SwitchSceneSinglePeer>b__0));
						this.<>1__state = 1;
						return true;
					}
				...
				if (i <= 0)
				{
					List<NetworkObject> sceneObjects = networkSceneManagerDefault.FindNetworkObjects(CS$<>8__locals1.loadedScene, true, false);
					finished(sceneObjects);
					return false;
				}
