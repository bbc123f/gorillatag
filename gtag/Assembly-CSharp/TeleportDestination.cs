...
public class TeleportDestination : MonoBehaviour
{
	...
	public event Action<TeleportDestination> Deactivated
	{
		...
		add
		{
			...
			{
				action2 = action;
				Action<TeleportDestination> value2 = (Action<TeleportDestination>)Delegate.Combine(action2, value);
				action = Interlocked.CompareExchange<Action<TeleportDestination>>(ref this.Deactivated, value2, action2);
			}
		...
		remove
		{
			...
			{
				action2 = action;
				Action<TeleportDestination> value2 = (Action<TeleportDestination>)Delegate.Remove(action2, value);
				action = Interlocked.CompareExchange<Action<TeleportDestination>>(ref this.Deactivated, value2, action2);
			}
			while (action != action2);
		}
