...
namespace GorillaNetworking
{
	public class PlayFabTitleDataCache : MonoBehaviour
	{
		...
		[CompilerGenerated]
		private bool <UpdateDataCo>b__21_0(PlayFabTitleDataCache.DataRequest request)
		{
			string text;
			if (this.titleData.TryGetValue(request.Name, out text))
			{
				request.Callback.SafeInvoke(text);
				return true;
			}
			return false;
		}
		...
		[CompilerGenerated]
		private sealed class <UpdateDataCo>d__21 : IEnumerator<object>, IEnumerator, IDisposable
		{
			[DebuggerHidden]
			public <UpdateDataCo>d__21(int <>1__state)
			{
				...
				try
				{
					...
					if (num != 0)
					{
						...
						else
						{
							...
							else
							{
								...
								{
									string text6;
									if (playFabTitleDataCache.titleData.TryGetValue(request.Name, out text6))
									{
										request.Callback.SafeInvoke(text6);
										return true;
									}
					...
					else
					{
						...
						string text5 = JsonMapper.ToJson(new Dictionary<string, object>
						{
							...
							{ "key", playFabTitleDataCache.titleDataKey },
							{ "data", dictionary2 }
						});
