...
public static class FusionScalableIMGUI
{
	...
	public static ValueTuple<float, float, int, int, float> ScaleGuiSkinToScreenHeight()
	{
		...
		int num6 = (int)(flag ? ((num2 - (float)(num3 * 2)) * 0.07f) : (num * 0.4f));
		RectOffset rectOffset = new RectOffset(0, 0, num4, num4);
		FusionScalableIMGUI._scalableSkin.button.fontSize = num6;
		FusionScalableIMGUI._scalableSkin.button.margin = rectOffset;
		FusionScalableIMGUI._scalableSkin.label.fontSize = num6;
		FusionScalableIMGUI._scalableSkin.label.padding = new RectOffset(num3, num3, num3, num3);
		FusionScalableIMGUI._scalableSkin.textField.fontSize = num6;
		...
		return new ValueTuple<float, float, int, int, float>(num, num2, num3, num4, num5);
	}
