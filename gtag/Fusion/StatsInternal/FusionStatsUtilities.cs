...
namespace Fusion.StatsInternal
{
	public static class FusionStatsUtilities
	{
			...
			get
			{
				if (FusionStatsUtilities._cachedGraphVisualizationNames == null)
				{
					...
					for (int i = 0; i < names.Length; i++)
					{
						...
						try
						{
							text = ((DescriptionAttribute)typeFromHandle.GetMember(names[i])[0].GetCustomAttributes(typeof(DescriptionAttribute), false)[0]).Description;
						}
						catch
						{
							text = names[i];
						}
						FusionStatsUtilities._cachedGraphVisualizationNames.Add(text);
					}
				}
				...
				if (FusionStatsUtilities._circle32Texture == null)
				{
					...
					for (int i = 0; i < 64; i++)
					{
						for (int j = 0; j < 64; j++)
						{
							...
							float num2 = ((num > 64.0) ? 0f : ((num < 63.0) ? 1f : ((float)(64.0 - num))));
		...
		public static Dropdown CreateDropdown(this RectTransform rt, float padding, Color fontColor)
		{
			...
			RectTransform rectTransform3 = rectTransform2.CreateRectTransform("Content", false).ExpandTopAnchor(null).SetOffsets(0f, 0f, -150f, 0f)
				.CreateRectTransform("Item", true)
			...
			Image image2 = rectTransform3.CreateRectTransform("Item Background", true).gameObject.AddComponent<Image>();
			Image image3 = rectTransform3.CreateRectTransform("Item Checkmark", true).SetAnchors(0.05f, 0.1f, 0.1f, 0.9f).SetOffsets(0f, 0f, 0f, 0f)
				.gameObject.AddComponent<Image>();
			image3.sprite = FusionStatsUtilities.CircleSprite;
			image3.preserveAspect = true;
			Text text = rectTransform3.CreateRectTransform("Item Label", true).SetAnchors(0.15f, 0.9f, 0.1f, 0.9f).SetOffsets(0f, 0f, 0f, 0f)
				.AddText("Sample", TextAnchor.UpperLeft, fontColor);
			text.alignment = TextAnchor.MiddleLeft;
			text.resizeTextMaxSize = 24;
			toggle.targetGraphic = image2;
