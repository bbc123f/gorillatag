...
public class BodyDockPositions : MonoBehaviourPunCallbacks
{
	...
	public BodyDockPositions.DockingResult ToggleWithHandedness(string transferrableItemName, bool isLeftHand, bool bothHands)
	{
		...
		if (isLeftHand)
		{
			dropPositions = (((this.allObjects[list[0]].dockPositions & BodyDockPositions.DropPositions.LeftArm) != BodyDockPositions.DropPositions.None) ? BodyDockPositions.DropPositions.RightArm : BodyDockPositions.DropPositions.LeftBack);
		}
		else
		{
	...
	public void RefreshTransferrableItems()
	{
		...
		foreach (int num in this.objectsToEnable)
		{
			this.EnableTransferrableGameObject(this.myRig.ActiveTransferrableObjectIndex(num), this.myRig.TransferrableDockPosition(num), this.myRig.TransferrablePosStates(num));
		}
	...
	{
		int num = this.myRig.LeftThrowableProjectileIndex;
		if (num < 0)
		{
			num = this.PreviousLeftHandThrowableIndex;
			if (num < 0 || this.PreviousLeftHandThrowableDisabledTime < Time.time)
			{
				return null;
			}
		}
		return this.leftHandThrowables[num];
	}

	internal GameObject GetRightHandThrowable()
	{
		int num = this.myRig.RightThrowableProjectileIndex;
		if (num < 0)
		{
			num = this.PreviousRightHandThrowableIndex;
			if (num < 0 || this.PreviousRightHandThrowableDisabledTime < Time.time)
			{
				return null;
			}
		}
