...
public class PassthroughStyler : MonoBehaviour
{
	...
	private sealed class <FadeTo>d__33 : IEnumerator<object>, IEnumerator, IDisposable
	{
		...
		bool IEnumerator.MoveNext()
		{
			...
			timer += Time.deltaTime;
			float num2 = Mathf.Clamp01(timer / duration);
			if (passthroughStyler._currentStyle == OVRPassthroughLayer.ColorMapEditorType.ColorLut)
			{
				passthroughStyler._passthroughLayer.SetColorLut(passthroughStyler._passthroughColorLut, Mathf.Lerp(blend, passthroughStyler._savedBlend * styleValueMultiplier, num2));
			}
			else
			{
				passthroughStyler._passthroughLayer.SetBrightnessContrastSaturation(Mathf.Lerp(brightness, passthroughStyler._savedBrightness * styleValueMultiplier, num2), Mathf.Lerp(contrast, passthroughStyler._savedContrast * styleValueMultiplier, num2), Mathf.Lerp(saturation, passthroughStyler._savedSaturation * styleValueMultiplier, num2));
			}
			passthroughStyler._passthroughLayer.edgeColor = Color.Lerp(edgeCol, new Color(passthroughStyler._savedColor.r, passthroughStyler._savedColor.g, passthroughStyler._savedColor.b, passthroughStyler._savedColor.a * styleValueMultiplier), num2);
