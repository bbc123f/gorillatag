...
public class SpawnerPrototype<T> : SimulationBehaviour, IPlayerJoined, IPlayerLeft, ISpawned, ISceneLoadDone where T : Component, ISpawnPointPrototype
{
	...
	public void Spawned()
	{
		...
		{
			NetworkObject networkObject = this.TrySpawn(this.Runner, this.Runner.LocalPlayer);
			this.RegisterPlayerAndObject(this.Runner.LocalPlayer, networkObject);
		}
	}
	...
	private void PlayerJoined(NetworkRunner runner, PlayerRef player)
	{
		...
		NetworkObject networkObject = this.TrySpawn(runner, player);
		this.RegisterPlayerAndObject(player, networkObject);
	}
	...
	public void TrySpawnAll()
	{
		...
		while (instancesEnumerator.MoveNext())
		{
			...
			if (networkRunner.IsRunning && networkRunner.IsServer)
			{
				...
				{
					NetworkObject networkObject = this.TrySpawn(networkRunner, playerRef);
					this.RegisterPlayerAndObject(playerRef, networkObject);
				}
			}
		}
	}
