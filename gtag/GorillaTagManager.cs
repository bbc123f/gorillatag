...
public class GorillaTagManager : GorillaGameManager
{
	...
	public void UpdateState()
	{
		if (PhotonNetwork.IsMasterClient)
		{
			...
			else if (this.isCurrentlyTag && GameMode.ParticipatingPlayers.Count >= this.infectedModeThreshold)
			{
				...
				this.AddInfectedPlayer(GameMode.ParticipatingPlayers[num2]);
			...
			else if (!this.isCurrentlyTag && this.currentInfected.Count == 0)
			{
				...
				this.AddInfectedPlayer(GameMode.ParticipatingPlayers[num4]);
			}
	...
	public IEnumerator InfectionEnd()
	{
		...
		if (!this.isCurrentlyTag && this.waitingToStartNextInfectionGame)
		{
			...
			if (participatingPlayers.Count > 0)
			{
				...
				this.AddInfectedPlayer(participatingPlayers[num]);
	...
	public void UpdateInfectionState()
	{
		...
		{
			if (!this.currentInfected.Contains(player))
			{
				this.allInfected = false;
				break;
			}
	...
	public override void ReportTag(Player taggedPlayer, Player taggingPlayer)
	{
		if (PhotonNetwork.LocalPlayer.IsMasterClient)
		{
			...
			if (b == 1)
			{
				object[] array = new object[] { taggingPlayer.UserId, taggedPlayer.UserId };
				PhotonNetwork.RaiseEvent(1, array, raiseEventOptions, SendOptions.SendReliable);
				return;
			}
			if (b == 2)
			{
				object[] array2 = new object[] { taggingPlayer.UserId, taggedPlayer.UserId, num };
				PhotonNetwork.RaiseEvent(2, array2, raiseEventOptions, SendOptions.SendReliable);
			}
		}
	}
	...
	public override void HitPlayer(Player taggedPlayer)
	{
		if (PhotonNetwork.LocalPlayer.IsMasterClient)
		{
			...
			if (b == 1)
			{
				object[] array = new object[] { taggedPlayer.UserId, taggedPlayer.UserId };
				PhotonNetwork.RaiseEvent(1, array, raiseEventOptions, SendOptions.SendReliable);
				return;
			}
			if (b == 2)
			{
				object[] array2 = new object[] { taggedPlayer.UserId, taggedPlayer.UserId, num };
				PhotonNetwork.RaiseEvent(2, array2, raiseEventOptions, SendOptions.SendReliable);
			}
		}
	}
