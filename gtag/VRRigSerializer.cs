...
internal class VRRigSerializer : GorillaWrappedSerializer<InputStruct>, IFXContextParems<HandTapArgs>, IFXContextParems<GeoSoundArg>
{
	...
	{
		Debug.Log("VrRigSerializer - On istantiate Setup");
		...
		{
			string text = "Try get Vr rig:";
			RigContainer rigContainer = this.rigContainer;
			Debug.Log(text + ((rigContainer != null) ? rigContainer.ToString() : null));
			outTargetObject = this.rigContainer.gameObject;
			outTargetType = typeof(VRRig);
			this.vrrig = this.rigContainer.Rig;
			string text2 = "DEBUG: returning True, rig = ";
			VRRig vrrig = this.vrrig;
			Debug.Log(text2 + ((vrrig != null) ? vrrig.ToString() : null));
			return true;
		}
		Debug.Log("DEBUG: returning False");
		return false;
	}
	...
	protected override void OnSuccesfullySpawned(PhotonMessageInfoWrapped info)
	{
		...
		Debug.Log("<color=green> PLAYER SPAWN SUCCESS");
		...
		if (NetworkSystem.Instance.IsObjectLocallyOwned(base.gameObject))
		{
			Debug.Log("Succesfully spawned VRRigSerializer");
		}
	}
	...
	protected unsafe static void RPC_InitializeNoobMaterial@Invoker(NetworkBehaviour behaviour, SimulationMessage* message)
	{
		...
		num += 4;
		float num5 = num4;
		float num6 = (float)(*(int*)(data + num)) * 0.001f;
		num += 4;
		float num7 = num6;
		RpcInfo rpcInfo = RpcInfo.FromMessage(behaviour.Runner, message, RpcHostMode.SourceIsServer);
		behaviour.InvokeRpc = true;
		((VRRigSerializer)behaviour).RPC_InitializeNoobMaterial(num3, num5, num7, rpcInfo);
	}
	...
	protected unsafe static void RPC_RequestMaterialColor@Invoker(NetworkBehaviour behaviour, SimulationMessage* message)
	{
		...
		RpcInfo rpcInfo = RpcInfo.FromMessage(behaviour.Runner, message, RpcHostMode.SourceIsServer);
		behaviour.InvokeRpc = true;
		((VRRigSerializer)behaviour).RPC_RequestMaterialColor(num3, rpcInfo);
	}
	...
	protected unsafe static void RPC_PlayHandTap@Invoker(NetworkBehaviour behaviour, SimulationMessage* message)
	{
		...
		RpcInfo rpcInfo = RpcInfo.FromMessage(behaviour.Runner, message, RpcHostMode.SourceIsServer);
		behaviour.InvokeRpc = true;
		((VRRigSerializer)behaviour).RPC_PlayHandTap(num3, flag2, num5, rpcInfo);
	}
